...
# =========================
# Itinerary Agent - low token version
# =========================
...
2. Respect the user's destination, duration and budget.
3. Use the available information above.
4. Do not invent unavailable flight, hotel, price or weather details.
5. If information is unavailable, simply omit it or mention it briefly.
6. Include activities, meals and transportation suggestions.
7. Keep the response concise and easy to follow.
8. Produce a draft ready for human review.
"""
    ...
    response = llm.invoke(
        [
            SystemMessage(
                content=(
                    "You are a concise expert travel planner. "
                    "Return only the useful travel itinerary."
                )
            ),
            HumanMessage(content=prompt),
        ]
    ...
    approval_request = (
        "Please review the generated draft itinerary. Approve it to create "
        "the final polished plan, or provide feedback for revision."
    )
